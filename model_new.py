...
#,bounds=(-5,5))
# w0: intercept
model.w0 = Var(within=Integers, bounds = (-5,5))
#bounds=(-5*model.p,5*model.p)) # TBC
...
def error_(model):
     return sum(log(1+exp(-model.Y[i]*(sum(model.omega_leq[i,j] + model.omega_geq[i,j] for j in model.P) + model.w0))) for i in model.N)
