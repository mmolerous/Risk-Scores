# -*- coding: utf-8 -*-
"""
Created on Mon Apr 22 17:38:33 2024
...
#         import shelve
#         shelf_round = shelve.open(path + "/results_auc/" + "results_" +  "ipopt" + "_round" + "_TL" + str(TL) + "_n" + str(n) + "_p"  + str(p) + "_k" + str(k0) + ".dat")
#         r_w0_round = shelf_round["r_w0_round"] 
#         r_w_leq_round = shelf_round["r_w_leq_round"] 
#         r_w_geq_round = shelf_round["r_w_geq_round"]
#         r_t_round = shelf_round["r_t_round"]
#         r_alpha_round = shelf_round["r_alpha_round"]
#         r_b_round = shelf_round["r_b_round"]
#         r_omega_leq_round = shelf_round["r_beta_leq_round"] 
#         r_omega_geq_round = shelf_round["r_beta_geq_round"]
#         objective_value_round = shelf_round["objective_value_round"]
#         shelf_round.close()    
        
#         # This is code for giving an initial solution:
#         instance.w0 =  int(r_w0_round)
#         for i in range(0,len(X)):
#             for j in range(len(X.columns)):
#                 instance.b[i+1,j+1] = int(r_b_round[i,j])
#                 instance.omega_leq[i+1,j+1] = int(r_omega_leq_round[i,j])
#                 instance.omega_geq[i+1,j+1] = int(r_omega_geq_round[i,j])     
#         for j in range(len(X.columns)):
#             instance.w_leq[j+1] = int(r_w_leq_round[j])
#             instance.w_geq[j+1] = int(r_w_geq_round[j])
#             instance.t[j+1] = r_t_round[j]
#             instance.alpha[j+1] = int(r_alpha_round[j])
        
#         import sys
#         orig_stdout = sys.stdout
#         f = open(path + "/results_auc/" + which_solver + "_ISipopt" + "_TL" + str(TL) + "_n" + str(n) + "_p" + str(p) + "_k" + str(k0) + '_all.txt', 'w')
        
#         sys.stdout = f
            
#         t0 = time.time()
#         results = opt.solve(instance,tee=True,keepfiles=True, logfile = path + "/results_auc/" +  which_solver + "_ISipopt" + "_TL" + str(TL) + "_n" + str(n) + "_p" + str(p) + "_k" + str(k0) + "_log.log")
#         t1 = time.time()
#         T = t1-t0
...
#             import shelve
#             shelf = shelve.open(path + "/results_auc/" + "results_" + which_solver + "_ISipopt" + "_TL" + str(TL) + "_n" + str(n) + "_p"  + str(p) + "_k" + str(k0) + ".dat")
#             shelf["r_w0"] = r_w0
...
#    for n in [1000,2000,3000,4000,5000,10000,11000,12000,13000,14000,15000,20000]:
    for n in [2000,3000,4000,11000,12000,13000,14000]:
        np.random.seed(0)
        #n = 200
        p = 1
        k0 = 1
